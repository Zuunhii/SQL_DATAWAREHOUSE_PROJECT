...
BEGIN
    ...
    BEGIN TRY
		...
		-- CRM_CUST_INFOR----------------------------------------------------------------------------
		PRINT '>> Truncate Table: silver.crm_cust_infor'
		TRUNCATE TABLE silver.crm_cust_infor
		PRINT '>> Insert Table: silver.crm_cust_infor'
		...
		ELSE sls_price
		END AS sls_price
		FROM [DWH].[bronze].[crm_sales_details]
		SET @end_time = GETDATE();
        PRINT '>> Load Duration: ' + CAST(DATEDIFF(SECOND, @start_time, @end_time) AS NVARCHAR) + ' seconds';
        PRINT '>> -------------';
